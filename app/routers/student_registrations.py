# ==================== STUDENT APIS ====================
# app/routes/student_registrations.py
...
async def submit_form(
    ...
    try:
        ...
        current_time = datetime.utcnow()
        if db_form.status != FormStatus.OPEN:
            raise HTTPException(
...
async def get_student_submission(
    ...
    try:
        ...
        logger.error(f"Error fetching student submission: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_form_status(
    ...
    try:
        ...
        return {
            "form_id": form_id,
            "form_status": db_form.status.value,
        ...
        logger.error(f"Error fetching form status: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# This can be run as a background task or scheduled job
async def auto_lock_expired_submissions(db: Session):
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error auto-locking submissions: {str(e)}")